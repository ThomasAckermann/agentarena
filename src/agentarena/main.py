...
def main():
    ...
    parser = argparse.ArgumentParser(description="AgentArena - A 2D shooting game with configurable AI agents")
    parser.add_argument("--player", choices=["manual", "random", "ml"], default="manual", 
                        help="Type of agent to control the player")
    parser.add_argument("--enemy", choices=["random", "ml"], default="random", 
                        help="Type of agent to control the enemies")
    parser.add_argument("--ml-model", type=str, help="Path to ML model file (required if using ML agent)")
