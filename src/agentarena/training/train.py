...
from agentarena.training.reward_functions import calculate_reward, RewardType

def train(episodes=1000, model_save_freq=100, render=False, checkpoint_path=None, reward_type=RewardType.ADVANCED):
    """Train the ML agent"""
    print(f"Starting ML agent training with {reward_type.value} reward function...")
    ...
    if render:
        ...
        pygame.display.set_caption(f"AgentArena ML Training - {reward_type.value}")
    ...
    try:
        for episode in range(1, episodes + 1):
            ...
            # Print progress
            print(f"Episode {episode}/{episodes} - Steps: {step} - Reward: {episode_reward:.2f} - Avg Reward: {avg_reward:.2f} - Epsilon: {player_agent.epsilon:.4f}")
...
def evaluate(model_path, episodes=10, render=True):
    ...
    print(f"Evaluation complete - Avg Reward: {avg_reward:.2f} - Avg Episode Length: {avg_length:.2f}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train or evaluate an ML agent for AgentArena")
    parser.add_argument("--mode", choices=["train", "evaluate"], default="train", help="Mode to run in")
    parser.add_argument("--episodes", type=int, default=1000, help="Number of episodes to run")
    parser.add_argument("--render", action="store_true", help="Render the game during training/evaluation")
    parser.add_argument("--model-path", help="Path to the model file (required for evaluation, optional for training)")
    parser.add_argument("--save-freq", type=int, default=100, help="How often to save the model during training")
    parser.add_argument("--reward-type", choices=[r.value for r in RewardType], default=RewardType.ADVANCED.value,
