...
def plot_training_results(results_file):
    ...
    axes[0].plot(episodes[window_size-1:], smoothed_rewards, 'r-', linewidth=2, label=f'{window_size}-episode moving average')
    axes[0].legend()
...
def plot_comparison(results_files, labels=None):
    ...
    for i, results in enumerate(all_results):
        ...
        if len(rewards) > window_size:
            smoothed_rewards = np.convolve(rewards, np.ones(window_size)/window_size, mode='valid')
            axes[0].plot(episodes[window_size-1:], smoothed_rewards, linewidth=2, label=labels[i])
        else:
    ...
    for i, results in enumerate(all_results):
        ...
        if len(lengths) > window_size:
            ...
            axes[1].plot(episodes[window_size-1:], smoothed_lengths, linewidth=2, label=labels[i])
        else:
