...
class MLAgent(Agent):
    ...
    def __init__(self, name="MLAgent", learning_rate=0.001, gamma=0.99, epsilon=1.0, epsilon_min=0.01, epsilon_decay=0.995, is_training=True):
        super().__init__(name)
        ...
        self.directions = [None, Direction.UP, Direction.DOWN, Direction.LEFT, Direction.RIGHT,
        ...
        # State representation size (will be determined after first observation)
    ...
    def _initialize_model(self, state_size):
        ...
        self.model = DQNModel(state_size, self.n_actions)
    ...
    def encode_observation(self, observation):
        ...
        for enemy in observation['enemies']:
            ...
            enemy_features.extend([rel_x, rel_y, distance, angle, enemy_health])
        
        # Pad if there are fewer enemies than expected
        max_enemies = 3  # Maximum number of enemies based on config
        expected_enemy_features = max_enemies * 5  # 5 features per enemy
        ...
        # Process bullets
        bullet_features = []
        for bullet in observation['bullets'][:5]:  # Limit to nearest 5 bullets
            # Calculate relative position to player
        ...
        max_bullets = 5
        expected_bullet_features = max_bullets * 5  # 5 features per bullet
    ...
    def _game_action_to_idx(self, action):
        ...
        direction_idx = self.directions.index(action.direction) if action.direction in self.directions else 0
    ...
    def get_action(self, observation):
        ...
        # Initialize model if this is the first observation
    ...
    def load_model(self, path):
        ...
        self._initialize_model(self.state_size)
