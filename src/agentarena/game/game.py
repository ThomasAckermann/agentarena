...
class Game:
    ...
    def create_enemies(self) -> None:
        for _ in range(self.enemy_count):
            enemy_position = [
                random.randint(0, self.config.display_width - self.config.block_width),
                random.randint(
                    0, self.config.display_height - self.config.block_height
                ),
            ]
    ...
    def apply_action(
        ...
        if action.direction is not None:
            ...
            old_x = player.x
            old_y = player.y
            new_x = player.x + self.dt * player.speed * dx
            new_y = player.y + self.dt * player.speed * dy
        ...
        if action.is_shooting is True:
            ...
            if player.ammunition > 0 and player.cooldown == 0:
                bullet_width = int(self.config.block_width / 2)
                bullet_height = int(self.config.block_height / 2)

                # Calculate center of player
                center_x = player.x + (player.width - bullet_width) / 2
                center_y = player.y + (player.height - bullet_height) / 2

                # Optional: Offset bullet slightly in shooting direction
                offset = 5  # You can tweak this
                center_x += dx * offset
                center_y += dy * offset
    ...
    def move_bullets(self):
        ...
        for bullet in self.bullets:
            ...
            if not (0 <= bullet.x <= self.config.display_width) or not (
                0 <= bullet.y <= self.config.display_height
            ):
                continue
    ...
    def load_textures(self):
        self.textures = {
            ...
            "bullet": pygame.transform.scale(
                ...
                (self.config.block_width // 2, self.config.block_height // 2),
            ),
